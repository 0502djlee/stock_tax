...
class Transaction:
    def __init__(self, symbol, shares, date_acquired, date_sold, proceeds, cost_basis, accrued_market_discount, wash_sale_amount, gain, length):
        ...
        if(accrued_market_discount == "N/A"):
...
class Pdf2F8949:
    ...
    def text_to_pdf(self, length, name, taxpayer_id, year):
        ...
        pdf.output(length + "_transactions_" + "results.pdf", 'F')
...
"""
    def __write_transactions_to_txt_file(self, out_file, transactions, header):
        ...
        total_sales = total_cost_basis = total_wash = total_gain = 0.0
        ...
        out_file.write(f"\ntotal_{header}_sales({total_sales:9.7}) - total_{header}_cost_basis({total_cost_basis:9.7}) + total_{header}_wash({total_wash:9.7}) = {total_calc_gain:9.7} should be equal to total_{header}_gain({total_gain:9.7})\n\n")
"""
"""
    ...
    def __set_docx_table_font_size(self, table, size):
    ...
    def __set_docx_cell_font_size(self, cell, size):
        paragraphs = cell.paragraphs
        for paragraph in paragraphs:
    ...
    def get_docx_cells_time(self):
        return self.__docx_get_cells_time

    def __write_table_one_row(self, cells, shares, symbol, date_acquired, date_sold, proceeds, cost_basis, cost_instruction, wash, gain):
        cells[0].text = str("{0:>9.7}".format(shares)) + " " + symbol
        self.__set_docx_cell_font_size(cells[0], self.__table_font_size)
        cells[1].text = date_acquired
        self.__set_docx_cell_font_size(cells[1], self.__table_font_size)
        cells[2].text = date_sold
        self.__set_docx_cell_font_size(cells[2], self.__table_font_size)
        cells[3].text = str("{0:>9.7}".format(proceeds))
        self.__set_docx_cell_font_size(cells[3], self.__table_font_size)
        cells[4].text = str("{0:>9.7}".format(cost_basis))
        self.__set_docx_cell_font_size(cells[4], self.__table_font_size)
        cells[5].text = cost_instruction
        self.__set_docx_cell_font_size(cells[5], self.__table_font_size)
        cells[6].text = str("{0:>9.7}".format(wash))
        self.__set_docx_cell_font_size(cells[6], self.__table_font_size)
        cells[7].text = str("{0:>9.7}".format(gain))
        self.__set_docx_cell_font_size(cells[7], self.__table_font_size)

    def __write_table_last_line(self, cells, sum_sales, sum_cost_basis, sum_wash, sum_gain):
        cells[0].text = "Totals"
        self.__set_docx_cell_font_size(cells[0], self.__table_font_size)
        cells[1].text = " "
        self.__set_docx_cell_font_size(cells[1], self.__table_font_size)
        cells[2].text = " "
        self.__set_docx_cell_font_size(cells[2], self.__table_font_size)
        cells[3].text = str("{0:>9.7}".format(sum_sales))
        self.__set_docx_cell_font_size(cells[3], self.__table_font_size)
        cells[4].text = str("{0:>9.7}".format(sum_cost_basis))
        self.__set_docx_cell_font_size(cells[4], self.__table_font_size)
        cells[5].text = " "
        self.__set_docx_cell_font_size(cells[5], self.__table_font_size)
        cells[6].text = str("{0:>9.7}".format(sum_wash))
        self.__set_docx_cell_font_size(cells[6], self.__table_font_size)
        cells[7].text = str("{0:>9.7}".format(sum_gain))
        self.__set_docx_cell_font_size(cells[7], self.__table_font_size)

    def __write_transactions_to_docx(self, document, transactions, header):
        table = document.add_table(rows=self.__lines_per_page, cols=self.__items_in_line)
        table.style = 'Medium Grid 1 Accent 1' # set your style, look at the help documentation for more help
        ...
        total_sales = total_cost_basis = total_wash = total_gain = 0.0
        for transaction in transactions:
            get_cells_start_time = time.time()
            cells = table.rows[i].cells
            self.__docx_get_cells_time += time.time() - get_cells_start_time
            i += 1
            shares = transaction.get_shares()
            symbol = transaction.get_symbol()
            date_acquired = transaction.get_date_aquired()
            date_sold = transaction.get_date_transaction()
            proceeds = transaction.get_proceeds()
            cost_basis = transaction.get_cost_basis()
            cost_instruction = transaction.get_cost_instruction()
            wash = transaction.get_amount_adjustment()
            gain = transaction.get_gain()

            self.__write_table_one_row(cells, shares, symbol, date_acquired, date_sold, proceeds, cost_basis, cost_instruction, wash, gain)
            ...
            if (i >= self.__lines_per_page - 1):
                #self.__set_docx_table_font_size(table, self.__table_font_size)
                cells = table.rows[i].cells
                self.__write_table_last_line(cells, sum_sales, sum_cost_basis, sum_wash, sum_gain)

                #document.add_paragraph(f"{'totals':>37} {sum_sales:9.7} {sum_cost_basis:9.7} {' ':4} {sum_wash:9.7} {sum_gain:9.7}\n")
                ...
                sum_sales = sum_cost_basis = sum_wash = sum_gain = 0.0
                i = 0
                row_iter = iter(table.rows)

        #self.__set_docx_table_font_size(table, self.__table_font_size)
        if (i > 0):
            #document.add_paragraph(f"{'totals':>37} {sum_sales:9.7} {sum_cost_basis:9.7} {' ':4} {sum_wash:9.7} {sum_gain:9.7}\n")
            for idx in range(i, self.__lines_per_page):
                row = table.rows[idx]
                for cell in row.cells:
                    cell.text = " "
            cells = table.rows[self.__lines_per_page - 1].cells
            self.__write_table_last_line(cells, sum_sales, sum_cost_basis, sum_wash, sum_gain)
            self.__set_docx_table_font_size(table, self.__table_font_size)
    ...
    def write_docx(self):
        ...
        for item in self.__f8949_short_transactions.values():
            short_list = short_list + item
        self.__write_transactions_to_docx(document, short_list, "short")
        long_list = []
        for item in self.__f8949_long_transactions.values():
            long_list = long_list + item
        self.__write_transactions_to_docx(document, long_list, "long")

        document.save(self.__outfile_name + '.docx') # Save document
...
if __name__ == "__main__":
    ...
    pdf_reader = Pdf2F8949(["TY_2020_1099_Consolidated_9072_1.pdf", "TY_2020_1099_Consolidated_9072_2.pdf"], "result_file.txt")
    ...
    #pdf_reader.text_to_pdf()
    #pdf_reader.print_test()

"""
    start_time = time.time()   
    pdf_reader.write_txt_file()
    print("---- %s seconds for writing txt file" % (time.time() - start_time))

    start_time = time.time()   
    pdf_reader.write_docx()
    print("---- %s seconds for writing docx file" % (time.time() - start_time))

    print("---- %s seconds for get cells time" % pdf_reader.get_docx_cells_time())
"""
